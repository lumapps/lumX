(function() {
    ...
    function LxDropdownController(
        ...
        lxDropdown.closeOnClick = angular.isDefined(lxDropdown.closeOnClick)
            ? lxDropdown.closeOnClick
            : true;
        lxDropdown.effect = angular.isDefined(lxDropdown.effect)
            ? lxDropdown.effect
            : 'expand';
        lxDropdown.escapeClose = angular.isDefined(lxDropdown.escapeClose)
            ? lxDropdown.escapeClose
            : true;
        lxDropdown.hasToggle = false;
        lxDropdown.isOpen = false;
        lxDropdown.overToggle = angular.isDefined(lxDropdown.overToggle)
            ? lxDropdown.overToggle
            : false;
        lxDropdown.position = angular.isDefined(lxDropdown.position)
            ? lxDropdown.position
            : 'left';
        lxDropdown.minOffset =
            angular.isUndefined(lxDropdown.minOffset) ||
            lxDropdown.minOffset < 0
                ? 8
                : lxDropdown.minOffset;
        ...
        $scope.$on('lx-dropdown__close', function(_event, _params) {
            if (
                _params.uuid === lxDropdown.uuid &&
                lxDropdown.isOpen &&
                (!_params.documentClick ||
                    (_params.documentClick && lxDropdown.closeOnClick))
            ) {
                closeDropdownMenu();
            }
        });
        ...
        function closeDropdownMenu() {
            ...
            if (lxDropdown.hasToggle) {
                dropdownToggleElement = angular.isString(dropdownToggle)
                    ? angular.element(dropdownToggle)
                    : dropdownToggle;
            ...
            if (
                lxDropdown.effect === 'expand' ||
                lxDropdown.effect === 'fade'
            ) {
                    ...
                    complete: function() {
                        ...
                        $scope.$apply(function() {
                            ...
                            if (lxDropdown.escapeClose) {
                                LxEventSchedulerService.unregister(
                                    idEventScheduler
                                );
        ...
        function getAvailableHeight() {
            ...
            var direction;
            var dropdownToggleElement = angular.isString(dropdownToggle)
                ? angular.element(dropdownToggle)
                : dropdownToggle;
            var dropdownToggleHeight = dropdownToggleElement.outerHeight();
            var dropdownToggleTop =
                dropdownToggleElement.offset().top -
            ...
            if (lxDropdown.overToggle) {
                ...
                availableHeightOnBottom =
                    windowHeight - (dropdownToggleTop + dropdownToggleHeight);
            }
        ...
        function initDropdownPosition() {
            ...
            if (angular.isDefined(lxDropdown.width)) {
                if (lxDropdown.width.indexOf('%') > -1) {
                    dropdownMenuWidth =
                        dropdownToggleWidth *
                        (lxDropdown.width.slice(0, -1) / 100);
            ...
            if (lxDropdown.position === 'left') {
                ...
                dropdownMenuLeft =
                    dropdownMenuLeft <= lxDropdown.minOffset
                        ? lxDropdown.minOffset
                        : dropdownMenuLeft;
                ...
                dropdownMenuRight =
                    windowWidth -
                    dropdownToggleElement.offset().left -
                    dropdownToggleWidth;
                dropdownMenuRight =
                    dropdownMenuRight > windowWidth - lxDropdown.minOffset
                        ? windowWidth - lxDropdown.minOffset
                        : dropdownMenuRight;
            } else if (lxDropdown.position === 'center') {
                dropdownMenuLeft =
                    dropdownToggleElement.offset().left +
                    dropdownToggleWidth / 2 -
                    dropdownMenuWidth / 2;
                dropdownMenuLeft =
                    dropdownMenuLeft <= lxDropdown.minOffset
                        ? lxDropdown.minOffset
            ...
            if (availableHeight.direction === 'top') {
                dropdownMenu.css({
                    bottom: lxDropdown.overToggle
                        ? windowHeight -
                          dropdownToggleTop -
                          dropdownToggleHeight
                        : windowHeight - dropdownToggleTop + ~~lxDropdown.offset
                });

                return availableHeight.top;
            } else if (availableHeight.direction === 'bottom') {
                dropdownMenu.css({
                    top: lxDropdown.overToggle
                        ? dropdownToggleTop
                        : dropdownToggleTop +
                          dropdownToggleHeight +
                          ~~lxDropdown.offset
        ...
        function openDropdownMenu() {
            ...
            if (!lxDropdown.hover) {
                ...
                scrollMask.on(
                    'click wheel touchmove ontouchstart',
                    closeDropdownMenu
                );
            }
            ...
            if (lxDropdown.escapeClose) {
                idEventScheduler = LxEventSchedulerService.register(
                    'keyup',
                    onKeyUp
                );
            }
            ...
            openTimeout = $timeout(function() {
                var availableHeight =
                    initDropdownPosition() - ~~lxDropdown.offset;
                 ...
                 */
                dropdownContentWatcher = $scope.$watch(
                    function watcherDropdownContent() {
                        return dropdownMenu
                            .find('.dropdown-menu__content')
                            .html();
                    },
        ...
        function updateDropdownMenuHeight() {
            ...
            var dropdownMenuHeight = dropdownMenu
                .find('.dropdown-menu__content')
                .outerHeight();

            if (
                availableHeight[availableHeight.direction] -
                    ~~lxDropdown.offset <=
                dropdownMenuHeight
            ) {
                if (availableHeight.direction === 'top') {
            ...
            element.on(mouseEvent, function(_event) {
                ...
                if (
                    ...
                    ('ontouchstart' in window &&
                        angular.element($window).outerWidth() <= 480)
                ) {
                    return;
                }
